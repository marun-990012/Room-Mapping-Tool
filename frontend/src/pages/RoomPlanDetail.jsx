...
export default function RoomPlanDetail() {
  ...
  useEffect(() => {
    ...
    map.on(L.Draw.Event.DELETED, async (e) => {
      e.layers.eachLayer(async (layer) => {
        try {
          await axios.delete(`/api/room/delete/${layer.options.roomId}`, {
            headers: { Authorization: localStorage.getItem("token") },
          });
    ...
    <div className="min-h-screen bg-gray-100 px-6 py-6">
      <div className="flex gap-3">
        <button
          ...
          className="bg-gray-500 px-3 py-1 rounded border-2 border-white text-white"
        >
          Back
        </button>
        <button
          ...
          className="flex items-center justify-evenly gap-2 bg-green-500 hover:bg-green-700 transition-all duration-300 px-3 py-1 rounded border-2 border-green-200 text-white"
        >
          <SquarePen size={16} />
          create new
        </button>
